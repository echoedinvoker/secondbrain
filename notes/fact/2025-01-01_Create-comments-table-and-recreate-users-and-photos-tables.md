...
We will also recreate the `users` and `photos` tables to ensure that our database schema and data are consistent with the course.


```sql
DROP TABLE IF EXISTS photos; -- photos must be dropped first because of the foreign key constraint to users
DROP TABLE IF EXISTS users;

-- recreate the users and photos tables
CREATE TABLE users(
...
-- create the comments table which has foreign key constraints to both users and photos
CREATE TABLE comments (
  id SERIAL PRIMARY KEY,
  contents VARCHAR(240),
  user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
  --                                   ^^^^^^^^^^^^^^^^^  one-to-many relationship between comments and users
  photo_id INTEGER REFERENCES photos(id) ON DELETE CASCADE
  --                                     ^^^^^^^^^^^^^^^^^ one-to-many relationship between comments and photos

  --                                  so if its related photo or user is deleted, the comment will be deleted as well
);
 
-- insert some sample data for the further exercises
