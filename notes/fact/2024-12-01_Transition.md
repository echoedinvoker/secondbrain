    ...
    <script>
      tailwind.config = {
        theme: {
          extend: {
            transitionDuration: {
         // ^^^^^^^^^^^^^^^^^^ each Tailwindcss class has its own name, so we can extend it
              0: '0ms',
              2000: '2000ms',  // becuase class `duration-*` only goes up to 1000ms
                               // so we can add our custom duration values here
                               // then we can use `duration-2000` in our classes
            },
          },
        },
      }
  ...
  <body>
    ...
    <button
      class="px-8 py-2 m-24 text-white bg-blue-500 rounded transition-colors duration-700 hover:bg-blue-700"
                                                      <!-- ^^^^^^^^^^^^^^^^^ ^^^^^^^^^^^^ `transition` and `transition-*` has default duration of 150ms, but we can change it by `duration-*` -->
                                                      <!-- ^ -->
    ...
    <button
      class="px-8 py-2 m-24 text-white bg-blue-500 rounded transition ease-in-out delay-150 duration-2000 hover:-translate-y-1 hover:scale-110 hover:bg-indigo-500"
                                                                 <!-- ^^^^^^^^^^^ `trantision` and `transition-*` has default timing fn, but we can change it by some classes such as `ease-in-out` -->
    >
      Click me
    </button>
    ...
    <img
      ...
      class="hover:transform hover:rotate-180 hover:scale-75 hover:skew-x-12 transition"
        <!-- ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Tailwindcss split each transform effect into separate classes, but need to start with `transform` class -->
    />
  </body>
</html>
...
<!-- Transition Property
  ...
  transition	    this is for some common properties for transition, it's different from `transition-all`, because it doesn't include all properties
...
<!-- we can use other classes to modify the transition effect, they are three groups: `duration-*`, `ease-*`, `delay-*`: -->
<!-- 
  Duration
...
<!-- TRANSFORMS -->
<!-- Tailwindcss split transform effects to separate classes, but need to start with `transform` class -->
